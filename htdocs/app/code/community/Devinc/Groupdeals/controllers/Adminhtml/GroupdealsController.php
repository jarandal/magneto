...
{ 
    protected function _isAllowed(){ 
        return Mage::getSingleton('admin/session')->isAllowed('admin/groupdeals');
    }
	...
	public function saveAction() {
		...
		if ($data = $this->getRequest()->getPost()) {	   
			...
			try {				
				...
				if (Mage::helper('groupdeals')->getMagentoVersion()>1420) {
				...
				if ($this->getRequest()->getParam('groupdeals_id', false) || (!$this->getRequest()->getParam('groupdeals_id', false) && !$this->getRequest()->getParam('id', false))) {
		    		...
		    		Mage::getModel('groupdeals/observer')->refreshIndexes();
				}
    ...
    {
		$product = $this->_initProduct();
        $fileName   = urldecode($this->getRequest()->getParam('csv_excel_name')).' Coupons.csv';
