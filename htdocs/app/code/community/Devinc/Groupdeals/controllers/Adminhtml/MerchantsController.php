...
class Devinc_Groupdeals_Adminhtml_MerchantsController extends Mage_Adminhtml_Controller_Action
{
	protected function _isAllowed(){ 
		if (Mage::getSingleton('admin/session')->isAllowed('admin/groupdeals/merchants') || Mage::getSingleton('admin/session')->isAllowed('admin/groupdeals/merchant_info')) {
			return true;
		}
	    return false;
	}
			...
			try {
				...
				if(isset($_FILES['merchant_logo']['name']) && $_FILES['merchant_logo']['name']!='') {
					try {    
					     ...
					     $_replace = array(' ',',','-');
                         $_name = str_replace($_replace, '_', $_FILES['merchant_logo']['name']);
						
						 $uploader->save($path, $_name);
                         $data['merchant_logo'] = 'groupdeals/merchants/logo/'.$_name;
					} catch (Exception $e) {
