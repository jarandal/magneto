...
{   
    ...
    protected function _prepareLayout()
    {
        ...
        if (!$crcId) {
            $crcId = Mage::getModel('groupdeals/crc')->getMainCrc($this->getGroupdeal()->getId())->getId();
        }
        if ($crcId && $breadcrumbsBlock = $this->getLayout()->getBlock('breadcrumbs')) {
            ...
            $breadcrumbsBlock->addCrumb('groupdeals', array(
				...
				'link'=>"javascript:popup.showPopup(false);"
            ));
    ...
    public function allowFacebookComments() {
    	if (Mage::helper('groupdeals')->displayInIe()) {
    		return Mage::getStoreConfig('groupdeals/facebook_connect/enable_facebook_comments');
    	} else {
    		return false;
    	}
    }
