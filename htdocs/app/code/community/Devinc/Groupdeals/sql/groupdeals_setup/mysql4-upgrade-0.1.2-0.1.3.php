...
if (count($couponCollection)) {
	foreach ($couponCollection as $coupon) {
		...
		$item = Mage::getModel('sales/order_item')->getCollection()->addFieldToFilter('order_id', $coupon->getOrderId())->addFieldToFilter('product_id', $productId)->getFirstItem();		
		if ($item && $item->getId()){
			$coupon->setOrderItemId($item->getId())->save();
		}
	}
}
...
$installer->setConfigData('groupdeals/facebook_connect/enable_facebook_comments',	1);
