...
{	
    ...
    {		
		return Mage::getModel('license/module')->merchantVerification($observer);
	}	
	
	//refresh groupdeal indexes
    public function refreshIndexes()
    {
    	$dealIds = Mage::getModel('groupdeals/groupdeals')->getCollection()->setOrder('groupdeals_id', 'ASC')->getColumnValues('groupdeals_id'); 
        $dealProductIds = Mage::getModel('groupdeals/groupdeals')->getCollection()->setOrder('groupdeals_id', 'ASC')->getColumnValues('product_id'); 
        $dealIdsArray = array();
        $i = 0;
        foreach ($dealIds as $dealId) {
            $dealIdsArray[$dealProductIds[$i]] = $dealId;
            $i++;
        }
    	$_urlRewriteCollection = Mage::getModel('core/url_rewrite')->getCollection()->addFieldToFilter('product_id', array('in' => $dealProductIds))->addFieldToFilter('target_path', array('like' => 'catalog/product/view/id/%'));
		$resource = Mage::getSingleton('core/resource');
	    $writeConnection = $resource->getConnection('core_write');
		if (Mage::helper('groupdeals')->isMagentoEnterprise()) {
			$table = $resource->getTableName('enterprise_url_rewrite');
		} else {
			$table = $resource->getTableName('core_url_rewrite');
		}

		if(count($_urlRewriteCollection)>0){
	 		foreach ($_urlRewriteCollection as $item) {
	 			$_rewriteId = $item->getUrlRewriteId();
	  			$_groupdealsId = $dealIdsArray[$item->getProductId()];

	 			if($item->getCategoryId() !== null && !Mage::helper('groupdeals')->isMagentoEnterprise()){
	  				$_currentTarget = $item->getTargetPath();
	 				$_newTarget = str_replace('catalog/', 'groupdeals/', $_currentTarget);
	 				$_newTargetCategory = str_replace('category/', 'groupdeals_id/'.$_groupdealsId.'/category/', $_newTarget);

				    $query = "UPDATE {$table} SET target_path = '{$_newTargetCategory}' WHERE url_rewrite_id = ".(int)$_rewriteId;
				    $writeConnection->query($query);
	 			} else {
	  				$_currentTarget = $item->getTargetPath();
	 				$_newTarget = str_replace('catalog/', 'groupdeals/', $_currentTarget);
	 				$_newTargetFinal = $_newTarget."/groupdeals_id/".$_groupdealsId;
	 				
				    $query = "UPDATE {$table} SET target_path = '{$_newTargetFinal}' WHERE url_rewrite_id = ".(int)$_rewriteId;
				    $writeConnection->query($query);
	 			}
	 		}
		}   	
	...
	//delete deal if product is deleted from catalog->manage products
	public function deleteDeal($observer)
    {
		$productId = $observer->getEvent()->getProduct()->getId();		
		$groupdeals = Mage::getModel('groupdeals/groupdeals')->getCollection()->addFieldToFilter('product_id', $productId)->getFirstItem();		
		
		$model = Mage::getModel('groupdeals/groupdeals');		 
		$model->load($groupdeals->getId())
			->delete();
	}
    	...
    	if (Mage::app()->getStore()->getCode()!='admin') {
			...
			if ($path == '' || $path == '/' || $path == '/index.php' || $path == '/index.php/' || $path == '/home' || $path == '/home/') {					
				...
				if ($helper->isEnabled() && $city!='default') {    	 
					...
					} else if (strpos($redirectUrl, '?') === true) {
						//set city/region
						$crcId = (int)substr($redirectUrl, strpos($redirectUrl, '?crc=')+5,100);
						if ($crcId!='') {
		...
		foreach ($items as $item) {		
			...
			if ($groupdeal->getId()) {	
				...
				if ($item->getIsVirtual()) {
					...
					for ($i=0; $i<$count; $i++) {
						$coupon = Mage::getModel('groupdeals/coupons')->getCollection()->addFieldToFilter('order_item_id', $item->getId())->addFieldToFilter('status', array('neq'=>'voided'))->addFieldToFilter('redeem', array('neq'=>'used'))->getFirstItem();
	...
	public function reviewCartItem($observer) {	
		if (Mage::helper('groupdeals')->isEnabled()) {	
			...
			if ($groupdeal->getId()) {		
				$totalQty = $this->_getQuoteItemQtyForCheck($groupdeal, $product->getId(), $item->getId(), $item->getQty());     
			    $maxQty = $groupdeal->getMaximumQty();
			    
			    if ($product->getGroupdealStatus()!=Devinc_Groupdeals_Model_Source_Status::STATUS_RUNNING) {
			    	$message = Mage::helper('groupdeals')->__('The &#34;%s&#34; DEAL is no longer available for purchase.', $product->getName());			    	
                	$item->setHasError(true);
                	$item->setMessage($message);
			    	$quote->setHasError(true);
			    	$quote->addMessage($message);
			    } else if ($maxQty<$totalQty) {
			    	$message = Mage::helper('groupdeals')->__('The maximum order qty available for the &#34;%s&#34; DEAL is %s. Please take into account your previous purchases as well.', $product->getName(), $maxQty);
                	$item->setHasError(true);
                	$item->setMessage($message);
			    	$quote->setHasError(true);
			    	$quote->addMessage($message);
	...
	//retrieves the total product qty from the cart; taking into account the qty of associated products or custom options from the cart as well as the customers previous orders
    protected function _getQuoteItemQtyForCheck($groupdeal, $productId, $quoteItemId, $itemQty)
    {
        ...
        if (!isset($this->_checkedQuoteItems[$productId]['prev_orders_qty'])) {
			...
			if ($customerSession->isLoggedIn()) {
			    ...
			    $prevOrdersQty = Mage::getModel('groupdeals/groupdeals')->getGroupdealsSoldQty($groupdeal, $customer->getEmail());			    
			}
