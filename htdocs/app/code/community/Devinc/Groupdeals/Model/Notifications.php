				...
				foreach ($groupdealCollection as $groupdeal) {
					...
					{	
						...
						if ($store->getId()) {
							...
							if ($allowTargetMetNotification && $targetMet->getId()=='' && $groupdeal->getMinimumQty()!=0 && $groupdeal->getMinimumQty()<=$soldQty && in_array($websiteId, $productWebsiteIds) && count($subscriberIds)>0) {	
								$subscriberIdsString = implode(',', $subscriberIds);								
								Mage::getModel('groupdeals/notifications')
								    ->setGroupdealsId($groupdeal->getId())
								    ->setWebsiteId($websiteId)
