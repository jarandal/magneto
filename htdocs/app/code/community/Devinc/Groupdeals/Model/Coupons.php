...
{		
		...
		if (count($_couponsCollection)>0) {				
			...
			foreach ($_couponsCollection as $_coupon) {
				...
				if ($orderId = $orderItem->getOrderId()) {
					...
					$_coupon->setStatus('disabled')->save();	
				}
	...
	public function getCouponHtml($_groupdeal, $_storeId, $_coupon = null, $_orderItem = null, $_customerName = 'JOHN DOE')
	{
		...
		//get merchant logo
		$merchantLogo = '';
		if ($_groupdeal->getCouponMerchantLogo()==1 && $merchant->getId() && $merchant->getStatus()==1 && $merchant->getMerchantLogo()!='') {
		...
		$html = 
		   '<html>
		    <body style="font-family:Arial;">
				<table id="couponTable" style="background:#F0F1F3; padding:10px; font-size:12px; max-width:700px; width:100%;">
					<thead>
					    <tr style="background:#313131;">
					    	<th class="gd-logo">
					    		<img style="float:left;" src="'.$logo.'" alt="logo" />'.$merchantLogo.'
					    	</th>
					    </tr>
					</thead>
					<tbody>
					    <tr style="background:#FFFFFF;">
					    	<td class="gd-main">
					    		<div style="text-align:left; clear:both; margin:0 0 10px 0; width:100%;">
					    			<h3 class="gd-title">'.$product->getName().'</h3> 
					    			<span style="width:60%; font-size:13px; color:#333333; ">'.$redeemableContent.'</span>
					    			'.$giftContent.'
					    		</div>	
					    		<div id="gd-image"><img width="100%" style="border:2px solid #D6D7D9;" src="'.$productImage.'" alt="deal_image" /></div>
					    		<div id="gd-info">	
					    			<div style="margin-top:10px; clear:both; margin:0 0 3px; ">
					    			...
					    			<table width="100%" style="margin:0px; font-size:12px; border-top:1px dashed #DDDDDD; ">
					    			...
					    			$html .= '</table>';
					    			if ($merchant->getId() && $merchant->getStatus()==1){ 
					    				$html .='<div style="margin-top:3px; padding:3px 0 0 0px; border-top:1px dashed #DDDDDD;">';
					    				
					    				if ($_groupdeal->getCouponMerchantAddress()==1 && $addresses!='') {
					    					$html .= '<span style="color:#333333">'.Mage::helper('groupdeals')->translate(array('ADDRESS:'), $_storeId).'</span><br/>'.$addresses;
					    				...
					    				if ($_groupdeal->getCouponMerchantContact()==1) {
					    					$html .= '<table width="100%" style="font-size:12px; margin-top:3px; padding-top:3px; display:block; border-top:1px dashed #DDDDDD; " cellspacing="0" cellpadding="0">
					    		...
					    		if ($_groupdeal->getCouponFinePrint()==1 || $_groupdeal->getCouponHighlights()==1) {
					    			$html .= '<div style="float:left; width:100%; border-top:1px dashed #DDDDDD; margin:5px 0 0; padding:10px 0 0;">';
					    			if ($_groupdeal->getCouponFinePrint()==1) $html .= '<div style="float:left; width:49%"><h3 style="display:block; padding:0px; font-size:14px; font-weight:bold; margin:0; color:#333333;">'.Mage::helper('groupdeals')->translate(array('The Fine Print'), $_storeId).'</h3>'.$groupdealFinePrint.'</div>';
					    				
					    			if ($_groupdeal->getCouponHighlights()==1) $html .= '<div style="float:right; width:49%"><h3 style="display:block; padding:0px; font-size:14px; font-weight:bold; margin:0; color:#333333;">'.Mage::helper('groupdeals')->translate(array('Highlights'), $_storeId).'</h3>'.$groupdealHighlights.'</div>';
				...
				<style type="text/css">
					.gd-logo { padding:20px 25px; text-align:left; border-bottom:2px solid #F1461E; }
					.gd-logo img { width:150px; }
					.gd-main { padding:15px; color:#808080; }
					.gd-title { padding:0px; font-size:18px; font-weight:bold; margin:0px; color:#333333; }
					#gd-image { width: 46%; float:left; padding-bottom:10px; }
					#gd-info { width: 50%; float:right; }
				</style>
			</body>
			</html>';
