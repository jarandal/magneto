...
function codeAddress(address, letter, imagesPath) {
    geocoder.geocode( { 'address': address}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
			...
			var browserTypeIE = (navigator.userAgent.match(/msie/i) || navigator.userAgent.match(/trident/i)) ? true : false;
			var browserVersion = jQueryGD.browser.version;
			if (browserTypeIE && browserVersion<8) {
			    var data = marker;
			    marker = new ActiveXObject( 'Microsoft.XMLDOM');
			    marker.async = false;
			    marker.loadXML(data);
			}
        } else {
